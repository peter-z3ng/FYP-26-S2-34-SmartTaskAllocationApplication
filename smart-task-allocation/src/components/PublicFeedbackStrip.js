...
export default function PublicFeedbackStrip() {
    ...
    <section className="relative z-10 mx-auto max-w-6xl px-6 pb-28">
      <div className="rounded-[32px] border border-white/10 bg-white/[0.055] p-6 shadow-[0_24px_100px_rgba(0,0,0,0.28)] backdrop-blur md:p-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {loading ? (
            <article className="rounded-[24px] border border-white/10 bg-white/[0.06] p-5 text-sm font-bold text-white/70 md:col-span-3">
              Loading moderated feedback...
            </article>
          ) : null}

          {!loading && feedback.length === 0 ? (
            <article className="rounded-[24px] border border-white/10 bg-white/[0.06] p-5 text-sm leading-7 text-white/70 md:col-span-3">
              Public feedback will appear here after the Platform Admin reviews and publishes customer submissions.
            </article>
          ) : null}

          {feedback.map((item) => (
            <article key={item.id} className="motion-card rounded-[24px] border border-white/10 bg-[#071428]/76 p-5">
