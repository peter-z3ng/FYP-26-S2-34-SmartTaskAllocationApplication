...
const roleActions = {
  ...
  manager: [
    { label: "Review task board", href: "/manager", group: "Workspace" },
    { label: "Create or assign task", href: "/manager/tasks", group: "Tasks" },
...
function profileHref(actor) {
  if (actor === "manager") return "/manager/settings";
...
export default function TopInformationBar({ actor }) {
  ...
  async function loadNotifications() {
    try {
      ...
      setNotifications(result.items?.length ? result.items : []);
    } catch (loadError) {
    ...
    <div className="top-info-bar relative z-40 flex min-h-14 w-full items-center gap-4 bg-[#C7DDEB]/80 px-4 py-1 backdrop-blur-md sm:px-6 lg:px-8">
      ...
      <div className="flex min-w-0 shrink-0 items-center justify-end gap-3">
          ...
          >
            ...
            <span className="truncate">
              {notifications.length
                ? `Workspace activity: ${notifications.length} update${notifications.length === 1 ? "" : "s"} waiting`
                : "Workspace activity: no pending updates"}
            </span>
          </button>
              ...
              <div className="mt-3 space-y-2">
                ...
                {notifications.map((item) => (
                  <Link
                    ...
                    className="block rounded-lg bg-[#f8faff] p-3 transition hover:-translate-y-0.5 hover:bg-white"
                  >
        ...
        <div ref={profileMenuRef} className="relative">
          ...
          >
            {profile.avatarUrl ? (
              <Image src={profile.avatarUrl} alt="" fill sizes="56px" unoptimized className="object-cover" />
            ) : (
              <UserIcon />
            )}
          </button>
