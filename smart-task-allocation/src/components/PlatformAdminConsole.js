    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <Metric
          ...
          detail={`Avg ${data?.analytics?.averageRating ?? 0}/5`}
...
function RecordList({ title, rows, empty, action, actions }) {
  return (
    <section className="dashboard-card p-6">
        ...
        {rows.map((row) => (
          <article key={row.log_id} className="rounded-2xl border border-slate-200 bg-white/70 p-4">
            {row.details?.priority ? (
              <span className="mb-3 inline-flex rounded-full bg-teal-100 px-3 py-1 text-xs font-black uppercase tracking-[0.14em] text-teal-800">
                Priority support
              </span>
            ) : null}
  ...
  return (
    <article className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-2xl shadow-slate-950/20">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <DetailField label="Status" value={details.status || "Published"} />
