...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      if (width <= 480 && config[mobileKey]) nextValue = config[mobileKey];
      else if (width <= 768 && config[tabletKey]) nextValue = config[tabletKey];
      else if (width <= 1024 && config[desktopKey]) nextValue = config[desktopKey];
