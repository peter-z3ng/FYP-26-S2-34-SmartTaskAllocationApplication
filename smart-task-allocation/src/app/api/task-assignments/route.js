...
export async function GET(request) {
  try {
    ...
    let query = supabase
      ...
      .select(
        "assignment_id, assigned_at, status, user:user_id(user_id, username, email), task:task_id!inner(task_id, title, organization_id, start_datetime, end_datetime)",
      )
  ...
  try {
    ...
    if (mode === "auto") {
      ...
      let employeesQuery = supabase
        .from("user_account")
        .select("user_id, organization_id, username, email, account_status")
