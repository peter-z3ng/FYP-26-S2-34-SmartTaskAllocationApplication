  ...
  try {
    ...
    if (isManagerRoute(homeRoute)) {
      ...
      let query = supabase
        ...
        .select(
          "request_id, requested_at, status, user:user_id(user_id, username, email, subscription_tier), task:task_id!inner(task_id, title, organization_id, start_datetime, end_datetime)",
        )
...
export async function PATCH(request) {
  try {
    ...
    if (nextStatus === "Cancelled") {
      ...
      const feature = getPaidFeature("requestApproval");

      if (!isPaidTier(managerAccount?.subscription_tier)) {
        return NextResponse.json(
          {
            error: paidFeatureError(feature?.title ?? "Request approval"),
            feature: feature?.key ?? "requestApproval",
