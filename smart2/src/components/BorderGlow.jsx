...
const BorderGlow = ({
  ...
  useEffect(() => {
    ...
    function runPremiumSweep() {
      if (cancelled) return;

      const angleStart = 90;
      const angleEnd = 810; // 2 full rotations
      ...
      animateValue({
        ...
        onUpdate: (v) => {
          setCursorAngle(
            angleStart + (angleEnd - angleStart) * (v / 100)
          );
        },
      });

      // Soft fade out near the end
      animateValue({
        delay: 7200,
        duration: 1800,
        ease: easeInCubic,
        ...
        onEnd: () => {
          ...
          timeoutId = window.setTimeout(runPremiumSweep, 1200);
        },
      });
    }
