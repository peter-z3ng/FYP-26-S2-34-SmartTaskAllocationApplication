...
async function getMembersForTeam(supabase, team) {
  ...
  const [{ data: users, error: usersError }, availabilityByUserId, skillsByUserId, profilesByUserId, clockLogsByUserId] =
    await Promise.all([
      supabase
        .from("user_account")
        .select(
      ...
      getLatestClockLogsForUsers(supabase, userIds),
    ]);
  ...
  return normalizedMemberRows.map((member) => {
    ...
    return {
      ...
      clock: clockStatusFromLatestLog(clockLogsByUserId.get(member.user_id)),
