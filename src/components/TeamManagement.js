...
function isMemberClockedIn(member) {
  return Boolean(member?.clock?.clockedIn);
}

function memberClockLabel(member) {
  return isMemberClockedIn(member) ? "Clocked in" : "Clocked out";
}

function memberClockBadgeClass(member) {
  return isMemberClockedIn(member)
...
export default function TeamManagement() {
  ...
  function startRenameTeam(team) {
    setEditingTeamId(String(team.team_id));
  ...
  function openAssignPanel(member) {
    ...
    if (!isMemberClockedIn(member)) {
      setError(`${memberDisplayName(member)} is clocked out and cannot receive a new assignment.`);
      return;
    }
        ...
        {isTeamSidebarCollapsed ? (
          ...
          <div className="mt-5 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
            {teams.map((team) => {
              ...
              const isEditing = String(team.team_id) === editingTeamId;
              const isMenuOpen = String(team.team_id) === openTeamMenuId;
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        setOpenTeamMenuId((current) =>
                          current === String(team.team_id) ? "" : String(team.team_id),
                        );
                      }}
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-lg font-bold text-[#667085] transition hover:bg-white/60 ${
      ...
      <div className="min-h-0 overflow-y-auto bg-white px-8 py-3">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-5 2xl:grid-cols-6">
          {acceptedMembers.map((member) => {
            ...
            const clockedIn = isMemberClockedIn(member);
            ...
            return (
              ...
              >
                <div className="space-y-2">
                  ...
                  <div
                    className={`rounded-[12px] border px-3 py-2 text-center text-xs font-black ${memberClockBadgeClass(
                      member,
                    )}`}
                  >
                    {memberClockLabel(member)}
                  </div>
                  <Button
                    type="button"
                    variant="outline"
                    disabled={isCurrentUser || !clockedIn}
                  ...
                  >
                    {clockedIn ? "Assign task" : "Clocked out"}
                  </Button>
                </div>
              </GlassmorphismProfileCard>
            );
