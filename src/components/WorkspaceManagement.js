  ...
  async function duplicateWorkspace(workspace) {
    ...
    try {
      ...
      for (const task of sourceTasks) {
        const newGroupId = task.group_id != null ? groupIdMap.get(task.group_id) ?? null : null;
        const assignee = employees.find((employee) => employee.user_id === task.assigned_to);
        const canKeepAssignee = Boolean(assignee?.clockedIn || assignee?.clock?.clockedIn);
        await fetch("/api/tasks", {
          ...
          body: JSON.stringify({
            ...
            assignedTo: canKeepAssignee ? task.assigned_to : "",
      ...
      <aside className="relative overflow-visible bg-[#BBE1FA] px-3 py-4 shadow-md shadow-[#2563EB]">
        ...
        <div className="mt-5 space-y-2">
          {workspaces.map((workspace) => {
            ...
            const isEditing = workspace.workspace_id === editingWorkspaceId;
            const isMenuOpen = openWorkspaceMenuId === workspace.workspace_id;
...
function EligibleEmployeesDrawer({ employees, onClose }) {
  const eligibleEmployees = employees.filter(isEmployeeClockedIn).slice(0, 8);
      ...
      <div className="grid max-h-[calc(100vh-13rem)] gap-2 overflow-y-auto p-4">
        {eligibleEmployees.map((employee) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-black text-emerald-700">
                {getEmployeeClockLabel(employee)}
              </span>
            </span>
        ...
        {!eligibleEmployees.length ? (
          <p className="rounded-2xl border border-dashed border-white/40 px-3 py-10 text-center text-sm font-semibold text-[#667085]">
            No clocked-in employees available yet.
          </p>
        ) : null}
      </div>
    </OptimusGlowPanel>
            ...
            <label className="grid gap-2 text-xs font-bold uppercase tracking-wide text-white/45">
              ...
              >
                ...
                {employees.map((employee) => {
                  const clockedIn = isEmployeeClockedIn(employee);
                  ...
                  return (
                    <option
                      ...
                      disabled={!clockedIn}
                    >
                      {getDisplayName(employee)} - {getEmployeeClockLabel(employee)}
                    </option>
                  );
                })}
              </select>
...
function PeoplePickerCell({ employees, selectedUserId, onAssign }) {
  ...
  function assignDroppedEmployee(event) {
    ...
    const droppedEmployee = employees.find((employee) => employee.user_id === userId);

    if (droppedEmployee && isEmployeeClockedIn(droppedEmployee)) {
      onAssign(userId);
      closePicker();
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {selectedEmployee ? (
          <span
            className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black ${getEmployeeClockBadgeClass(
              selectedEmployee,
            )}`}
          >
            {getEmployeeClockLabel(selectedEmployee)}
          </span>
        ) : null}
      </button>
            ...
            {selectedEmployee ? (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span
                    className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-[11px] font-black ${getEmployeeClockBadgeClass(
                      selectedEmployee,
                    )}`}
                  >
                    {getEmployeeClockLabel(selectedEmployee)}
            ...
            {filteredEmployees.map((employee) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left hover:bg-[#f8faff] disabled:cursor-not-allowed disabled:opacity-55"
                  title={clockedIn ? "Assign employee" : "Employee is clocked out and cannot be assigned"}
                >
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-black ${getEmployeeClockBadgeClass(employee)}`}>
                    {getEmployeeClockLabel(employee)}
                  </span>
                </button>
              );
...
function isEmployeeClockedIn(employee) {
  return Boolean(employee?.clock?.clockedIn ?? employee?.clockedIn);
}

function getEmployeeClockLabel(employee) {
  return isEmployeeClockedIn(employee) ? "Clocked in" : "Clocked out";
}

function getEmployeeClockBadgeClass(employee) {
  return isEmployeeClockedIn(employee)
