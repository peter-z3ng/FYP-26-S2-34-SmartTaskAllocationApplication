...
const roleActions = {
  ...
  useradmin: [
    ...
    { label: "Review password reset requests", href: "/useradmin/password-resets", group: "Accounts" },
  ...
  platformadmin: [
    { label: "Review support inquiries", href: "/platformadmin/inquiries", group: "Support" },
    { label: "Review workflow feedback", href: "/platformadmin/feedback", group: "Feedback" },
    { label: "Review avatar uploads", href: "/platformadmin/avatar-reviews", group: "Profiles" },
    { label: "Manage plans", href: "/platformadmin/subscriptions", group: "Plans" },
...
export default function TopInformationBar({ actor }) {
  ...
  const [profile, setProfile] = useState({ email: "", name: "", avatarUrl: "" });
  const [now, setNow] = useState(null);

  const notificationItemsForActor = useMemo(() => {
    if (actor === "platformadmin") {
      return [
        {
          id: "support-queue",
          title: "Support queue",
          text: "Manager and Employee support requests arrive in Contact Inquiry Management.",
        },
        {
          id: "feedback-review",
          title: "Feedback review",
          text: "Workflow feedback waits for Platform Admin show or hide approval.",
        },
        {
          id: "avatar-review",
          title: "Avatar review",
          text: "User avatar uploads wait for compliance approval before profile updates.",
        },
        ...notificationItems,
      ];
    }
    ...
    return [
      {
        ...
        text: "Users who request account recovery are sent to Password Resets for admin handling.",
      },
      ...notificationItems,
    ];
  }, [actor]);
  ...
  function profileHref() {
    if (actor === "platformadmin") {
      return "/platformadmin/my-space";
    }

    if (actor === "useradmin") {
      return "/useradmin/my-space";
    }
      ...
      <div className="flex shrink-0 items-center justify-end gap-3">
        ...
        {isNotificationsOpen ? (
          <div className="absolute right-0 top-12 w-80 rounded-xl border border-white/60 bg-white/20 p-3 shadow-[0_18px_60px_rgba(7,24,59,0.16)] backdrop-blur-sm">
            ...
            <div className="mt-3 space-y-2">
              {notificationItemsForActor.map((item) => (
        ...
        <div className="relative">
          <button
            ...
            className="relative flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-[#0a2a66] text-white shadow-sm transition hover:bg-[#07183b]"
            aria-label="Profile"
            title="Profile"
          >
            {profile.avatarUrl ? (
          ...
          {isProfileOpen ? (
            <div className="absolute right-0 top-14 w-72 rounded-xl border border-white/60 bg-white/20 p-3 shadow-[0_18px_60px_rgba(7,24,59,0.16)] backdrop-blur-lg">
              <div className="flex items-center gap-3 rounded-lg bg-[#f8faff] p-3">
                <div className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#0a2a66] text-white">
                  {profile.avatarUrl ? (
                    <Image
                      src={profile.avatarUrl}
                      alt={`${profile.name || "User"} avatar`}
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-bold text-[#07183b]">{profile.name}</p>
