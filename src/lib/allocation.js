...
export function clockStatusFromLatestLog(latestLog) {
  const clockedIn = latestLog?.action === "Clock In";

  return {
    clockedIn,
    status: clockedIn ? "Clocked In" : "Clocked Out",
    latestLog: latestLog ?? null,
    latestClockAction: latestLog?.action ?? "Clock Out",
    latestClockAt: latestLog?.created_at ?? null,
  };
}

export async function getLatestClockLogForUser(supabase, userId) {
  const { data, error } = await supabase
    .from("activity_log")
    .select("*")
    .eq("user_id", userId)
...
export async function getLatestClockLogsForUsers(supabase, userIds) {
  if (!userIds.length) {
    return new Map();
  }
...
export async function getEmployeeClockStatus(supabase, userId) {
  const latestLog = await getLatestClockLogForUser(supabase, userId);
  return clockStatusFromLatestLog(latestLog);
}

export async function ensureEmployeeClockedIn(supabase, userId) {
  const clock = await getEmployeeClockStatus(supabase, userId);

  if (!clock.clockedIn) {
    return { error: "Employee is clocked out and cannot receive new assignments.", clock };
  }
...
async function checkClockedIn(supabase, userId) {
  const clock = await getEmployeeClockStatus(supabase, userId);

  return {
    ok: clock.clockedIn,
    reason: clock.clockedIn
      ? "Employee is clocked in and can receive work."
      : "Employee is clocked out and cannot receive new assignments.",
    clock,
  };
}
...
export async function evaluateEmployeeForTask(supabase, task, employee) {
  ...
  const [clock, availability, conflict, requirements] = await Promise.all([
    checkClockedIn(supabase, employee.user_id),
  ...
  const checks = [clock, availability, conflict, requirements];
  // Score stays intentionally explainable: one point each for clock, availability, conflicts, and requirements.
