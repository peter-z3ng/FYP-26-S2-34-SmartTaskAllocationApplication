...
const userAccounts = accountSeeds.map(([userId, username, email, roleId, accountStatus, departmentId], index) => ({
  ...
  subscription_tier: roleId === 3 ? "Enterprise" : index % 3 === 0 ? "Enterprise" : index % 3 === 1 ? "Team" : "Starter",
...
const activityLogs = [
  {
    log_id: "demo-feedback-published-1",
    user_id: "demo-manager",
    action: "User Feedback Submitted",
    details: JSON.stringify({
      name: "Alicia Tan",
      role: "Operations Manager",
      company: "Optima Demo Organization",
      rating: 5,
      category: "Task allocation",
      quote: "Automated allocation made shift planning easier to defend.",
      message:
        "The recommendation score and eligibility explanation help managers understand why a task is matched to a specific employee.",
      status: "Published",
      submittedBy: "demo-manager",
    }),
    created_at: new Date(Date.parse(now) + 35 * 60 * 1000).toISOString(),
  },
  {
    log_id: "demo-feedback-published-2",
    user_id: "demo-employee",
    action: "User Feedback Submitted",
    details: JSON.stringify({
      name: "Ben Carter",
      ...
      quote: "Available tasks are easier to find and request.",
      message:
        "The available-task list makes it clear which work matches my schedule, and the request flow is simple to follow.",
      status: "Published",
      submittedBy: "demo-employee",
    }),
    created_at: new Date(Date.parse(now) + 32 * 60 * 1000).toISOString(),
  },
  {
    log_id: "demo-feedback-pending-1",
    ...
    details: JSON.stringify({
      ...
      quote: "Clock in and clock out records are easier to audit.",
      message:
        "The time-clock panel gives a clear record of employee attendance, but I would like an export option in a future update.",
      status: "Pending",
      submittedBy: "demo-employee-2",
    }),
    created_at: new Date(Date.parse(now) + 28 * 60 * 1000).toISOString(),
  },
  {
    log_id: "demo-feedback-hidden-1",
    ...
    details: JSON.stringify({
      ...
      rating: 2,
      category: "General",
      quote: "Needs clearer onboarding copy.",
      message:
        "This demo entry is kept hidden so Platform Admin can verify that hidden feedback is not shown on the public feedback page.",
      status: "Hidden",
      submittedBy: "demo-employee-3",
    }),
    created_at: new Date(Date.parse(now) + 24 * 60 * 1000).toISOString(),
  },
];
...
function applyFilters(rows, filters) {
  return filters.reduce((nextRows, filter) => {
    ...
    if (filter.type === "ilike") {
      ...
      return nextRows.filter((row) => String(rowValue(row, filter.field) ?? "").toLowerCase().includes(pattern));
    }

    if (filter.type === "or") {
      return nextRows.filter((row) =>
        filter.conditions.some((condition) => String(rowValue(row, condition.field)) === String(condition.value)),
      );
    }
...
class DemoQueryBuilder {
  ...
  or(expression) {
    const conditions = String(expression)
      ...
      .filter((condition) => condition.field && condition.operator === "eq");
  ...
  insert(payload) {
    this.mode = "insert";
    const existingRows = tableData[this.tableName] ?? [];
    this.payload = (Array.isArray(payload) ? payload : [payload]).map((row, index) => {
      if (this.tableName === "activity_log" && !row.log_id) {
        return { ...row, log_id: `demo-log-${Date.now()}-${index + 1}` };
      }

      if (this.tableName === "task" && row.task_id == null) {
        const maxTaskId = Math.max(0, ...existingRows.map((entry) => Number(entry.task_id) || 0));
        return { ...row, task_id: maxTaskId + index + 1 };
      }

      if (this.tableName === "task_group" && row.group_id == null) {
        const maxGroupId = Math.max(0, ...existingRows.map((entry) => Number(entry.group_id) || 0));
        return { ...row, group_id: maxGroupId + index + 1 };
      }

      if (this.tableName === "team" && row.team_id == null) {
        const maxTeamId = Math.max(0, ...existingRows.map((entry) => Number(entry.team_id) || 0));
        return { ...row, team_id: maxTeamId + index + 1 };
      }

      if (this.tableName === "task_assignment" && row.assignment_id == null) {
        const maxAssignmentId = Math.max(
          0,
          ...existingRows.map((entry) => Number(entry.assignment_id) || 0),
        );
        return { ...row, assignment_id: maxAssignmentId + index + 1 };
      }
